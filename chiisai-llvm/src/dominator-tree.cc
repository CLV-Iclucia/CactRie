...
void DominatorTreeImpl::buildFromGraph(const SparseGraph&graph, size_t entry) {
  ...
  while (!workList.empty()) {
    ...
    for (auto succ : graph.successors(current)) {
      ...
      if (dom[succ] == newOutState) continue;
      dom[succ] = newOutState;
      if (inQueue[succ]) continue;
      workList.push(succ);
      inQueue.place(succ);
    }
  }
  ...
  for (auto u : std::views::iota(0u, graph.numNodes())) {
    if (u == entry) continue;
    if (graph.predecessors(u).size() < 2) continue;
...
void DominatorTree::buildFromCFG(const Function&func) {
  ...
  for (auto [bb, idx] : mystl::zip(func.basicBlocks(), std::views::iota(0u))) {
    blockToIndex[bb] = idx;
    indexToBlock[idx] = bb;
  }
