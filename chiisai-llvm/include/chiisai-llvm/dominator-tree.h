...
// I had planned to use Csr Matrix here...but let's just keep it simple and stupid
struct SparseGraph {
  SparseGraph() = default;
  explicit SparseGraph(std::vector<std::vector<uint32_t>> adj) : succ(std::move(adj)) {
...
struct DominatorTreeImpl {
  ...
  [[nodiscard]] const std::vector<uint32_t>& dominanceFrontier(size_t node) const {
...
struct DominatorTree {
  ...
  std::vector<Ref<BasicBlock>> dominanceFrontier(CRef<BasicBlock> node) const {
    ...
    std::transform(domTree.dominanceFrontier(blockToIndex.at(node)).begin(),
                   domTree.dominanceFrontier(blockToIndex.at(node)).end(),
                   result.begin(),
